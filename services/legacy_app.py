...
# =========================
# SQLite設定（Ver2.2 DB安定化）
# WAL固定／トランザクション整理／DBアクセス一本化／排他制御／整合性チェック
# =========================
...
def apply_sqlite_pragmas(conn: sqlite3.Connection, for_write: bool = False):
    """SQLite安定化のための基本PRAGMAを全接続に適用する。"""
    conn.execute(f"PRAGMA busy_timeout={DB_BUSY_TIMEOUT_MS};")
    conn.execute("PRAGMA foreign_keys=ON;")
    conn.execute("PRAGMA temp_store=MEMORY;")
    # WALは読み取りと書き込みの衝突を減らす。固定化のため毎回確認する。
    conn.execute("PRAGMA journal_mode=WAL;")
    # Streamlit Cloud等で速度と安全性のバランスを取る。
...
def _connect_hidamari_sqlite(for_write: bool = False) -> sqlite3.Connection:
    """DB接続の唯一の入口。直接 sqlite3.connect を呼ばない。"""
    ensure_dirs()
...
def get_hidamari_conn():
    """
    互換用の接続関数。
    新規コードでは hidamari_db_connection / hidamari_write_transaction を使う。
    """
...
def normalize_df_columns(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    """DataFrameに必要列をそろえる。"""
    if df is None:
...
def prepare_sqlite_dataframe(df: pd.DataFrame, columns: list, date_cols=None, unique_cols=None, sort_cols=None) -> pd.DataFrame:
...
def db_write_dataframe(df: pd.DataFrame, table_name: str, columns: list, date_cols=None, unique_cols=None, sort_cols=None):
    """DB書き込みの一本化関数。全保存処理はここを通す。"""
    table_name = validate_sqlite_identifier(table_name)
    work = prepare_sqlite_dataframe(df, columns, date_cols=date_cols, unique_cols=unique_cols, sort_cols=sort_cols)
    with hidamari_write_transaction() as conn:
        work.to_sql(table_name, conn, if_exists="replace", index=False)


def db_read_dataframe(table_name: str, columns: list, date_cols=None) -> pd.DataFrame:
...
def save_sqlite_table(df: pd.DataFrame, table_name: str, columns: list, date_cols=None, unique_cols=None, sort_cols=None):
    """互換用保存関数。内部では安定化済みDB書き込み関数を使う。"""
    db_write_dataframe(df, table_name, columns, date_cols=date_cols, unique_cols=unique_cols, sort_cols=sort_cols)


def load_sqlite_table(table_name: str, columns: list, date_cols=None) -> pd.DataFrame:
...
def run_db_integrity_check(auto_repair: bool = True) -> dict:
    """
    SQLite自動整合性チェック。
    quick_checkがNGの場合は警告を返す。auto_repair=Trueなら軽いVACUUM/ANALYZE相当まで行う。
    """
    ...
    try:
        ...
        if auto_repair and ok:
            # 破損修復ではなく、統計更新と軽いチェックポイントで安定化する。
            with hidamari_write_transaction() as conn:
                try:
...
def normalize_users_df(df):
    ...
    work = work[work["利用者名"] != ""].drop_duplicates(subset=["利用者名"], keep="first")
    return work.reset_index(drop=True)
...
def ensure_user_file():
    ...
    if USER_FILE.exists():
        ...
        if not df.empty:
            save_sqlite_table(df, SQLITE_TABLE_USERS, USER_COLUMNS, unique_cols=["利用者名"])
            return
...
def save_users(df):
    ...
    save_sqlite_table(work, SQLITE_TABLE_USERS, USER_COLUMNS, unique_cols=["利用者名"])
